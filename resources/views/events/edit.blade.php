                    ...
                    <form action="{{ route('events.update', $event) }}" method="POST" class="space-y-6">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div class="md:col-span-2">
                                ...
                                <textarea name="subjects" id="subjects" rows="2" class="w-full rounded-xl border-gray-300 shadow-sm focus:border-indigo-500 focus:ring focus:ring-indigo-200 transition-shadow">{{ old('subjects', $event->subjects) }}</textarea>
                            </div>

                            <!-- Elements à ramener -->
                            <div>
                                <label for="requirements" class="block text-sm font-bold text-gray-700 mb-1">Éléments à ramener par les élèves</label>
                                <textarea name="requirements" id="requirements" rows="3" class="w-full rounded-xl border-gray-300 shadow-sm focus:border-indigo-500 focus:ring focus:ring-indigo-200 transition-shadow">{{ old('requirements', $event->requirements) }}</textarea>
                            </div>

                            <!-- Guides -->
                            <div>
                                <label for="guides" class="block text-sm font-bold text-gray-700 mb-1">Guides des Applications IA</label>
                                <textarea name="guides" id="guides" rows="3" class="w-full rounded-xl border-gray-300 shadow-sm focus:border-indigo-500 focus:ring focus:ring-indigo-200 transition-shadow">{{ old('guides', $event->guides) }}</textarea>
                            </div>
                        </div>

                        <div class="flex items-center justify-end gap-4 pt-6 border-t border-gray-100">
                            <a href="{{ route('events.show', $event) }}" class="text-gray-500 hover:text-gray-700 font-medium">Annuler</a>
